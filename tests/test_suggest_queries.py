...
class TestSuggestQueriesErrors:
    ...
    def test_empty_index_returns_error(self, tmp_path):
        ...
        src.mkdir()
        store.mkdir()
        # Write a parseable file that yields no symbols (nexusmill-local:
        # README.md no longer qualifies — markdown headings ARE symbols since
        # f219d81, so a comment-only .py keeps this test on the empty-index path)
